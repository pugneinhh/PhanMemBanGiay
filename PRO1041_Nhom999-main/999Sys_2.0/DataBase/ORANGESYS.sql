...
GO
-- DEM SO HOA DON
IF OBJECT_ID('FNSOHDTRONGTHANG') IS NOT NULL
	DROP FUNCTION FNSOHDTRONGTHANG
GO
CREATE FUNCTION FNSOHDTRONGTHANG(@DAY DATE)
RETURNS INT
AS
BEGIN
	DECLARE @SOHD INT
...
END
GO
--NGAY MUA TRONG THANG
IF OBJECT_ID('FNNGAYMUA') IS NOT NULL
	DROP FUNCTION FNNGAYMUA
GO
CREATE FUNCTION FNNGAYMUA(@DAY DATE)
RETURNS DATE
AS
BEGIN
	DECLARE @NGAYMUA DATE
	SET @NGAYMUA = (SELECT CAST(NGAYMUA AS DATE) FROM HOADON WHERE CAST(NGAYMUA AS DATE) = @DAY AND MATT = 2 GROUP BY CAST(NGAYMUA AS DATE) )
	RETURN @NGAYMUA
END
GO
--SO LUONG MUA TRONG THANG
IF OBJECT_ID('FNSL') IS NOT NULL
	DROP FUNCTION FNSL
GO

CREATE FUNCTION FNSL(@DAY DATE)
RETURNS INT
AS
BEGIN
	DECLARE @SL INT
	SET @SL = (SELECT SUM(SOLUONG) FROM HOADON HD JOIN CT_HOADON CT ON CT.MAHD = HD.MAHOADON  WHERE CAST(NGAYMUA AS DATE) = @DAY AND MATT = 2 GROUP BY CAST(NGAYMUA AS DATE) )
	RETURN @SL
END
GO
--GIAM GIA TRONG THANG
IF OBJECT_ID('FNGIAMGIA') IS NOT NULL
	DROP FUNCTION FNGIAMGIA
GO
CREATE FUNCTION FNGIAMGIA(@DAY DATE)
RETURNS MONEY
AS
BEGIN
	DECLARE @GIAMGIA MONEY
	SET @GIAMGIA = (SELECT SUM(GIAMGIA) FROM HOADON WHERE CAST(NGAYMUA AS DATE) = @DAY AND MATT = 2 GROUP BY CAST(NGAYMUA AS DATE) )
	RETURN @GIAMGIA
END
GO
...
BEGIN	
	...
	SET @THANHCONG = (SELECT COUNT(MAHOADON) FROM HOADON
				WHERE CAST(NGAYMUA AS DATE) = @TODAY AND MATT = 2)
	---ĐÊM HD THANH CONG
	DECLARE @THATBAI INT
	SET @THATBAI = (SELECT COUNT(MAHOADON) FROM HOADON
				WHERE CAST(NGAYMUA AS DATE) = @TODAY AND MATT = 3)
	---ĐÊM TONG TIEN
...
AS BEGIN
	SELECT DISTINCT
		dbo.FNNGAYMUA(NGAYMUA) NGAYMUA,
		dbo.FNSOHDTRONGTHANG(NGAYMUA) SOHD,							
		dbo.FNSL(NGAYMUA) SOLUONG,
		LEFT(CONVERT(VARCHAR(50),SUM(dbo.FNGIAMGIA(NGAYMUA)),1),CHARINDEX('.',CONVERT(VARCHAR(50),SUM(dbo.FNGIAMGIA(NGAYMUA)),1)) - 1) GIAMGIA,
