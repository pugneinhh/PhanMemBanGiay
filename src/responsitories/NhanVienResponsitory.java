...
public class NhanVienResponsitory {
    ...
    public ArrayList<NhanVien> getNVLamByCV(String tenCV,String sdt){
        ArrayList<NhanVien> list = new ArrayList<>();
        String sql = "SELECT NHANVIEN.* FROM NHANVIEN JOIN CHUCVU ON NHANVIEN.IDCV=CHUCVU.ID WHERE NHANVIEN.TRANGTHAI=1 AND TEN LIKE ? OR SDT LIKE ?";
        ResultSet rs = JDBCHelper.executeQuery(sql,"%"+tenCV+"%","%"+sdt+"%");
    ...
    public ArrayList<NhanVien> getNVNghiByCV(String tenCV,String sdt){
        ArrayList<NhanVien> list = new ArrayList<>();
        String sql = "SELECT NHANVIEN.* FROM NHANVIEN JOIN CHUCVU ON NHANVIEN.IDCV=CHUCVU.ID WHERE NHANVIEN.TRANGTHAI=0 AND TEN LIKE ? OR SDT LIKE ?";
        ResultSet rs = JDBCHelper.executeQuery(sql,"%"+tenCV+"%","%"+sdt+"%");
